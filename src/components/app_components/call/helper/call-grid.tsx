...
import { MicOff, VideoOff, ChevronLeft, ChevronRight, Maximize2, Minimize2, Move } from "lucide-react"
...
export default function CallGrid({
  ...
  const AudioTile = ({ participant }: { participant: Participants }) => {
    const stream = participant.userId === currentUser.id ? localStream : remoteStreams[participant.userId]
    ...
    return (
      <div className="flex flex-col items-center justify-center p-4 bg-muted/30 rounded-xl min-h-[250px] min-w-[200px] max-w-[300px] max-h-[400px]">
        <div className="relative mb-3">
          <Avatar className={`h-16 w-16 ${isSpeaking ? "ring-3 ring-green-400" : ""}`}>
            <AvatarImage
              src={
  ...
  const VideoTile = ({
    ...
    const getTileClasses = () => {
      const baseClasses = "relative rounded-xl overflow-hidden transition-all duration-200 bg-muted cursor-pointer border border-zinc-200 "
      const ringClasses = isSpeaking ? "ring-3 ring-green-400" : isCurrentUser ? "ring-3 ring-blue-400" : ""

      if (isExpanded) {
        return `${baseClasses} ${ringClasses} w-full h-full`
      } else if (isInSidebar) {
        return `${baseClasses} ${ringClasses} aspect-video w-full max-h-24`
      } else if (isSmallView) {
        return `${baseClasses} ${ringClasses} aspect-video w-40 h-24 md:w-48 md:h-28`
      } else {
        return `${baseClasses} ${ringClasses} aspect-video w-full h-full min-h-[120px]`
      }
    }
        ...
        <div className="w-full h-full relative">
          {participant.video && stream ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-muted">
              <Avatar
                className={`${isExpanded ? "h-24 w-24" : isInSidebar ? "h-8 w-8" : isSmallView ? "h-12 w-12" : "h-16 w-16"}`}
              >
                <AvatarImage
                  src={
                    participant.user.profileImageUrl
  ...
  if (video && isOneOnOne) {
    ...
    return (
      <div className="flex-1 flex flex-col h-full w-full overflow-hidden relative justify-center items-center">
        <div className="flex-1 p-3 max-w-screen-lg min-w-[512px] md:min-w-[1024px] ">
          {main && <VideoTile participant={main} isExpanded={true} onClick={() => handleTileClick(main.userId)} />}
        </div>
        ...
        >
          <div className="relative group">
            {small && (
              <VideoTile participant={small} isSmallView={true} onClick={() => handleTileClick(small.userId)} />
            )}
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      <div className="flex-1 flex min-h-0">
        ...
        >
              ...
              {!video ? (
                <div className="flex flex-wrap gap-4 justify-center items-center max-w-5xl">
                  {currentPageParticipants.map((participant) => (
                    <AudioTile key={participant.userId} participant={participant} />
                  ))}
                </div>
              ) : (
          ...
          {video && currentUserParticipant && !isOneOnOne && expandedParticipant!==currentUserParticipant.userId && (
            <div
              className={`absolute z-20 ${
                currentUserPosition === "top-left"
                  ? "top-6 left-4"
